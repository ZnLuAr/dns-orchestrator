//! Cloudflare Provider integration testing
//!
//! Operation mode:
//! ```bash
//! CLOUDFLARE_API_TOKEN=xxx TEST_DOMAIN=example.com \
//!     cargo test -p dns-orchestrator-provider --test cloudflare_test -- --ignored --nocapture --test-threads=1
//! ```
...
#[tokio::test]
#[ignore = "integration test: requires CLOUDFLARE_API_TOKEN and TEST_DOMAIN"]
async fn test_cloudflare_validate_credentials() {
    skip_if_no_credentials!("CLOUDFLARE_API_TOKEN", "TEST_DOMAIN");

    let ctx = require_some!(TestContext::cloudflare(), "创建测试上下文失败");
...
async fn test_cloudflare_get_domain() {
    ...
    let mut ctx = require_some!(TestContext::cloudflare(), "创建测试上下文失败");
...
crud_test!(
    test_cloudflare_crud_mx_record,
    TestRecordType::Mx,
    "MX",
    common::generate_test_record_name
);
crud_test!(
    test_cloudflare_crud_txt_record,
    TestRecordType::Txt,
    "TXT",
    common::generate_test_record_name
);
crud_test!(
    test_cloudflare_crud_srv_record,
    TestRecordType::Srv,
    "SRV",
    common::generate_srv_test_record_name
);
crud_test!(
    test_cloudflare_crud_caa_record,
    TestRecordType::Caa,
    "CAA",
    common::generate_test_record_name
);
